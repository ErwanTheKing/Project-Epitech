...
static void display_sprite(main_radar_t *radar)
{
    ...
    sfTransform_scale(&transform, 1.6f, 1.6f);
    states.transform = transform;
    sfRenderWindow_drawSprite(radar->window, radar->main.background.sprite,
        &states);
    sfTransform_scale(&transform, 0.3f, 0.3f);
    states.transform = transform;
    sfRenderWindow_drawSprite(radar->window, radar->main.logo.sprite, &states);
}

static void display_text(main_radar_t *radar)
{
    sfRenderWindow_drawText(radar->window, radar->main.exit, NULL);
    sfRenderWindow_drawText(radar->window, radar->main.play, NULL);
}
