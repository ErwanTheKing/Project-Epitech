...
Test(len_nb, positive_case, .init = redirect_all_std)
{
    cr_assert_eq(len_nb(9876), 4);
}

Test(my_char_isnum, digit_and_nondigit, .init = redirect_all_std)
{
    cr_assert_eq(my_char_isnum('7'), 1);
    cr_assert_eq(my_char_isnum('a'), 0);
}

Test(my_char_isnum, boundary_nondigits, .init = redirect_all_std)
{
    cr_assert_eq(my_char_isnum('/'), 0);
    cr_assert_eq(my_char_isnum(':'), 0);
}

Test(my_put_nbr, recursive_and_negative_paths, .init = redirect_all_std)
{
    my_put_nbr(12345);
    my_putchar('\n');
    my_put_nbr(-42);
...
Test(my_put_nbr, single_digit_path, .init = redirect_all_std)
{
    my_put_nbr(7);
    cr_assert_stdout_eq_str("7");
}

Test(my_show_word_array, formatting, .init = redirect_all_std)
{
    char *tab[] = {"alpha", "beta", NULL};

    my_show_word_array(tab);
    cr_assert_stdout_eq_str("alpha\nbeta");
}

Test(openator, returns_null_on_missing_file, .init = redirect_all_std)
{
    cr_assert_null(openator("tests/does_not_exist_file"));
}

Test(openator, reads_existing_file, .init = redirect_all_std)
{
    char *content = openator("tests/testin_hello_file");

    cr_assert_not_null(content);
    cr_assert_str_eq(content, "hello_world");
    free(content);
}

Test(openator, empty_file_reports_error, .init = redirect_all_std)
{
    char *content = openator("tests/testing_empty_file");

    cr_assert_null(content);
    cr_assert_stderr_eq_str("FILE IS EMPTY\n");
}

Test(openator_helpers, check_open_paths, .init = redirect_all_std)
{
    cr_assert_eq(check_open(-1), FAILURE);
    cr_assert_eq(check_open(3), SUCCESS);
}

Test(openator_helpers, check_read_error_paths, .init = redirect_all_std)
{
    char *buffer = malloc(4);

    cr_assert_not_null(buffer);
    cr_assert_eq(check_read(buffer, -1), FAILURE);
    cr_assert_stderr_eq_str("READ FAILURE\n");
}

Test(openator_helpers, check_buffer_null, .init = redirect_all_std)
{
    cr_assert_eq(check_buffer(NULL), FAILURE);
    cr_assert_stderr_eq_str("BUFFER MEMORY ALLOCATION F");
}
