...
int lldi(prog_t *prog, cursor_t *cursor)
{
    ...
    int size = get_instruction_size(prog, cursor, 14);
    int reg = 0;
    int value = 0;

    if (cw_parse_arg_pos(prog, cursor, args, 14) == FAILURE)
        return size;
    if (read_index_arg(prog, cursor, &args[0]) == FAILURE)
        return size;
    if (read_index_arg(prog, cursor, &args[1]) == FAILURE)
        return size;
    value = cw_read_mem(prog,
        cursor->index + args[0].value + args[1].value, REG_SIZE);
