...
char *my_float_to_str(double nb, char *str, int i, int precision_bis)
{
    int nb_i = nb;
    int precision = my_compute_power_rec(10, precision_bis);
...
static int write_int_part(double nb, char *str, int count)
{
    int nw_nb = 0;
    int i = 0;
...
char *my_nbr_to_str(double nb, int precision)
{
    ...
    i = write_int_part(nb, str, count);
    str[i] = '.';
    my_float_to_str(nb, str, i + 1, precision);
    return str;
}
