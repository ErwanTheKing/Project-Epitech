...
static void create_slot_rect(window_t *win, inv_t *inv)
{
    ...
    inv->open = sfFalse;
    inv->rect[0] = create_rect(&size,
        &(sfVector2f){win->width / 2, win->height / 2},
        &(sfColor){110, 67, 32, 255});
    for (int i = 0; i < 8; i++) {
        size = (sfVector2f){x + i % 4 * (slot.x + 20),
            y + i / 4 * (slot.y + 20)};
        inv->rect[i + 1] = create_rect(&slot, &size, &sfWhite);
    }
    inv->rect[9] = create_rect(&(sfVector2f){win->width, win->height},
        &(sfVector2f){win->width / 2, win->height / 2},
...
static void init_item(window_t *wn, inv_t *inv)
{
    ...
    create_item(&inv->item[3], init_weapon(BANDAGE, 50, 1, create_entity("none",
                "assets/vacuum.png", &(sfVector2f){wn->width / 2, wn->height},
                &(sfVector2f){1.f, 1.f})), "Bandage", BANDAGE);
    inv->item[3].entity = create_entity("icon", "assets/bd_icon.png", &p, &p);
...
void free_inv(inv_t *inv)
{
    ...
    for (int i = 0; i < 10; i++) {
        if (inv->rect[i])
            sfRectangleShape_destroy(inv->rect[i]);
    }
    for (int i = 0; i < 8; i++) {
        if (inv->item[i].name)
            free(inv->item[i].name);
        destroy_weapon((weapon_t *)inv->item[i].data);
        destroy_entity(inv->item[i].entity);
        inv->item[i].data = NULL;
        inv->item[i].entity = NULL;
        inv->item[i].name = NULL;
    }
}
