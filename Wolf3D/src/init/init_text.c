...
static void init_main_menu_text(wolf_t *wolf, window_t *window)
{
    ...
    push_front(&wolf->list[MENU][TEXT],
        create_text(&(text_t){"option_btn", "OPTIONS", OPTIONS, TYPE_MENU,
            ...
            &(sfVector2f){1.5f, 1.5f}));
    push_front(&wolf->list[MENU][TEXT],
        create_text(&(text_t){"leave_btn", "LEAVE", LEAVE, TYPE_MENU, NULL,
                sfTrue, NULL, NULL}, wolf->data->font,
...
static void init_newgame_menu_text(wolf_t *wolf, window_t *window)
{
    push_front(&wolf->list[NEWGAME_MENU][TEXT],
        create_text(&(text_t){"solo_btn", "SOLO", SOLO, TYPE_MENU, NULL,
                sfTrue, NULL, NULL}, wolf->data->font,
            &(sfVector2f){window->width / 10, window->height / 2.2},
            &(sfVector2f){1.5f, 1.5f}));
    push_front(&wolf->list[NEWGAME_MENU][TEXT],
        create_text(&(text_t){"multi_btn", "MULTIJOUEUR", MULTI, TYPE_MENU,
                NULL, sfTrue, NULL, NULL}, wolf->data->font,
            &(sfVector2f){window->width / 10, window->height / 2.2 + 90},
            &(sfVector2f){1.5f, 1.5f}));
}

static void init_game_over_text(wolf_t *wolf, window_t *window)
{
    text_t *title = create_text(&(text_t){"game_over_title", "GAME OVER",
            -1, TYPE_MENU, NULL, sfTrue, NULL, NULL}, wolf->data->font,
        &(sfVector2f){window->width / 2.0f, window->height / 2.8f},
        &(sfVector2f){2.4f, 2.4f});
    text_t *main_menu = create_text(&(text_t){"game_over_main_menu",
            "MAIN MENU", 0, TYPE_MENU, NULL, sfTrue, NULL, NULL},
        wolf->data->font,
        &(sfVector2f){window->width / 2.0f, window->height / 1.85f},
        &(sfVector2f){1.5f, 1.5f});

    center_text_on_screen(title, window, window->height / 2.8f);
    if (title)
        sfText_setFillColor(title->text, sfColor_fromRGB(210, 35, 35));
    center_text_on_screen(main_menu, window, window->height / 1.85f);
    if (title)
        push_front(&wolf->list[GAME_OVER][TEXT], title);
    if (main_menu)
        push_front(&wolf->list[GAME_OVER][TEXT], main_menu);
}

void init_menu_text(wolf_t *wolf, window_t *window)
{
    init_main_menu_text(wolf, window);
    init_newgame_menu_text(wolf, window);
    init_game_over_text(wolf, window);
}
