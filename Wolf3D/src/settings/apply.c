...
static void apply_audio_settings(wolf_t *wolf)
{
    float master;
    float music;
    float sfx;
    ...
    if (!wolf || !wolf->settings)
        return;
    master = clamp_volume((float)wolf->settings->master_volume);
    music = clamp_volume(master *
        ((float)wolf->settings->music_volume / 100.0f));
    sfx = clamp_volume(master *
        ((float)wolf->settings->sfx_volume / 100.0f));
    for (int i = 0; i < 2; i++) {
