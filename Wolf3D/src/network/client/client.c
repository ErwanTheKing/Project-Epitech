...
static int client_recv_full_packet(client_t *client, network_packet_t *packet)
{
    size_t received;
    size_t total = 0;
    ...
    while (total < sizeof(network_packet_t)) {
        status = sfTcpSocket_receive(client->socket, buf + total,
            sizeof(network_packet_t) - total, &received);
...
int client_recv_packet(client_t *client, network_packet_t *packet)
{
    if (!client || !packet || !client->socket)
        return -1;
    return client_recv_full_packet(client, packet);
}
